# Taken from https://github.com/pomo-mondreganto/OVPNGen/blob/master/crypto_utils.py
...
def dump_file_in_mem(material, file_format=crypto.FILETYPE_PEM):
    if isinstance(material, crypto.X509):
        return crypto.dump_certificate(file_format, material)
    elif isinstance(material, crypto.PKey):
        return crypto.dump_privatekey(file_format, material)
    elif isinstance(material, crypto.X509Req):
        return crypto.dump_certificate_request(file_format, material)
    else:
